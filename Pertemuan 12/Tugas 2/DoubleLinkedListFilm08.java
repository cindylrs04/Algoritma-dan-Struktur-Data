public class DoubleLinkedListFilm08 {
    Film08 head;
    int size;

    public DoubleLinkedListFilm08() {
        head = null;
        size = 0;
    }
    ...
    public void addFirst(int id, String judul, double rate) {
        if (isEmpty()) {
            head = new Film08(null, id, judul, rate, null);
        } else {
            Film08 newNode = new Film08(null, id, judul, rate, head);
            head.prev = newNode;
            head = newNode;
        }
        size++;
    ...
    public void addLast(int id, String judul, double rate) {
        if (isEmpty()) {
            ...
            Film08 newNode = new Film08(current, id, judul, rate, null);
    ...
    public void add(int id, String judul, double rate, int index) throws Exception {
        if (isEmpty()) {
            ...
            if (current.prev == null) {
                Film08 newNode = new Film08(null, id, judul, rate, current);
                current.prev = newNode;
                head = newNode;
            } else {
                Film08 newNode = new Film08(current.prev, id, judul, rate, current);
    ...
    public Film08 findFilm(int key) {
        Film08 temp = head;
        while (temp != null) {
            if (temp.id == key) {
                break;
